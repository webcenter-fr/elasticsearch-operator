...
type ingressReconciler struct {
	multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *networkingv1.Ingress]
}

func newIngressReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *networkingv1.Ingress]) {
	return &ingressReconciler{
		MultiPhaseStepReconcilerAction: multiphase.NewMultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *networkingv1.Ingress](
			client,
			IngressPhase,
			IngressCondition,
			recorder,
...
func (r *ingressReconciler) Read(ctx context.Context, o *beatcrd.Filebeat, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*networkingv1.Ingress], res reconcile.Result, err error) {
