...
type metricbeatReconciler struct {
	multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *beatcrd.Metricbeat]
}

func newMetricbeatReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *beatcrd.Metricbeat]) {
	return &metricbeatReconciler{
		MultiPhaseStepReconcilerAction: multiphase.NewMultiPhaseStepReconcilerAction[*beatcrd.Filebeat, *beatcrd.Metricbeat](
			client,
			MetricbeatPhase,
			MetricbeatCondition,
			recorder,
...
func (r *metricbeatReconciler) Read(ctx context.Context, o *beatcrd.Filebeat, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*beatcrd.Metricbeat], res reconcile.Result, err error) {
