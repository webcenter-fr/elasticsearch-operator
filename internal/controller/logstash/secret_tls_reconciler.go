...
func (r *tlsReconciler) Diff(ctx context.Context, o *logstashcrd.Logstash, read multiphase.MultiPhaseRead[*corev1.Secret], data map[string]any, logger *logrus.Entry) (diff multiphase.MultiPhaseDiff[*corev1.Secret], res reconcile.Result, err error) {
	var (
		d         any
		needRenew bool
		isUpdated bool
	...
	if o.Spec.Pki.IsEnabled() {
		for _, s := range []*corev1.Secret{sPki, sCrt} {
