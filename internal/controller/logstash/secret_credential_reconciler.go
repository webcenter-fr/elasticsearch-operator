...
func newCredentialReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*logstashcrd.Logstash, *corev1.Secret]) {
...
func (r *credentialReconciler) Read(ctx context.Context, o *logstashcrd.Logstash, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*corev1.Secret], res reconcile.Result, err error) {
	s := &corev1.Secret{}
	sEs := &corev1.Secret{}
	...
	if o.Spec.ElasticsearchRef.IsManaged() {
		...
		if err = r.Client().Get(ctx, types.NamespacedName{Namespace: es.Namespace, Name: elasticsearchcontrollers.GetSecretNameForCredentials(es)}, sEs); err != nil {
			...
			logger.Warnf("Secret not found %s, try latter", elasticsearchcontrollers.GetSecretNameForCredentials(es))
			return read, reconcile.Result{RequeueAfter: 30 * time.Second}, nil
		}
	}
	...
	common.InjectTypeMeta(r.Client().Scheme(), expectedSecretCredentials...)
