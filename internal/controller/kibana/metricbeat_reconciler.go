...
type metricbeatReconciler struct {
	multiphase.MultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *beatcrd.Metricbeat]
}

func newMetricbeatReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *beatcrd.Metricbeat]) {
	return &metricbeatReconciler{
		MultiPhaseStepReconcilerAction: multiphase.NewMultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *beatcrd.Metricbeat](
			client,
			MetricbeatPhase,
			MetricbeatCondition,
			recorder,
...
func (r *metricbeatReconciler) Read(ctx context.Context, o *kibanacrd.Kibana, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*beatcrd.Metricbeat], res reconcile.Result, err error) {
