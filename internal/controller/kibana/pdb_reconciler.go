...
type pdbReconciler struct {
	multiphase.MultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *policyv1.PodDisruptionBudget]
}

func newPdbReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *policyv1.PodDisruptionBudget]) {
	return &pdbReconciler{
		MultiPhaseStepReconcilerAction: multiphase.NewMultiPhaseStepReconcilerAction[*kibanacrd.Kibana, *policyv1.PodDisruptionBudget](
			client,
			PdbPhase,
			PdbCondition,
			recorder,
...
// Read existing pdbs
func (r *pdbReconciler) Read(ctx context.Context, o *kibanacrd.Kibana, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*policyv1.PodDisruptionBudget], res reconcile.Result, err error) {
