...
type serviceReconciler struct {
	multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Metricbeat, *corev1.Service]
}

func newServiceReconciler(client client.Client, recorder record.EventRecorder) (multiPhaseStepReconcilerAction multiphase.MultiPhaseStepReconcilerAction[*beatcrd.Metricbeat, *corev1.Service]) {
	return &serviceReconciler{
		MultiPhaseStepReconcilerAction: multiphase.NewMultiPhaseStepReconcilerAction[*beatcrd.Metricbeat, *corev1.Service](
			client,
			ServicePhase,
			ServiceCondition,
			recorder,
...
func (r *serviceReconciler) Read(ctx context.Context, o *beatcrd.Metricbeat, data map[string]any, logger *logrus.Entry) (read multiphase.MultiPhaseRead[*corev1.Service], res reconcile.Result, err error) {
