...
func (t *ElasticsearchapiControllerTestSuite) TestUserReconciler() {
	...
	testCase.Steps = []test.TestStep{
		...
		doDeleteUserStep(),
		doCreateUserWithPasswordStep(),
		doUpdateUserPasswordStep(),
		doDeleteUserStep(),
		doCreateUserWithAutoGeneratedPassword(),
	}
...
func doMockUser(mockES *mocks.MockElasticsearchHandler) func(stepName *string, data map[string]any) error {
	return func(stepName *string, data map[string]any) (err error) {
		...
		mockES.EXPECT().UserGet(gomock.Any()).AnyTimes().DoAndReturn(func(name string) (*olivere.XPackSecurityUser, error) {
			...
			case "createWithAutoGeneratePassword":
		...
		mockES.EXPECT().UserCreate(gomock.Any(), gomock.Any()).AnyTimes().DoAndReturn(func(name string, policy *olivere.XPackSecurityPutUserRequest) error {
			...
			case "create":
				...
				data["isCreated"] = true
			case "createWithPassword":
				isCreatedWithPassword = true
				data["isCreatedWithPassword"] = true
				return nil
			case "createWithAutoGeneratePassword":
				isCreatedWithAutoGeneratePassword = true
				data["isCreatedWithAutoGeneratePassword"] = true
				return nil
			}
	...
	return test.TestStep{
		...
		Check: func(t *testing.T, c client.Client, key types.NamespacedName, o client.Object, data map[string]any) (err error) {
			...
			isTimeout, err := test.RunWithTimeout(func() error {
				...
				if b, ok := data["isCreatedWithPassword"]; ok {
					isCreated = b.(bool)
				}
				if !isCreated || user.GetStatus().GetObservedGeneration() == 0 {
					return errors.New("Not yet created")
			...
			assert.True(t, condition.IsStatusConditionPresentAndEqual(user.Status.Conditions, controller.ReadyCondition.String(), metav1.ConditionTrue))
			assert.True(t, *user.Status.IsSync)
			assert.NotEmpty(t, user.Status.PasswordHash)
...
func doUpdateUserPasswordStep() test.TestStep {
	return test.TestStep{
		...
		Do: func(c client.Client, key types.NamespacedName, o client.Object, data map[string]any) (err error) {
			...
			// Wait some time to reconcile secret
		...
		Check: func(t *testing.T, c client.Client, key types.NamespacedName, o client.Object, data map[string]any) (err error) {
			...
			isTimeout, err := test.RunWithTimeout(func() error {
				...
				if b, ok := data["isUpdatedPassword"]; ok {
					isUpdated = b.(bool)
				}
				if !isUpdated {
					return errors.New("Not yet updated")
			...
			oldPasswordHash := data["currentPasswordHash"].(string)
			assert.NotEqual(t, oldPasswordHash, user.Status.PasswordHash)
	...
	return test.TestStep{
		...
		Check: func(t *testing.T, c client.Client, key types.NamespacedName, o client.Object, data map[string]any) (err error) {
			...
			isTimeout, err := test.RunWithTimeout(func() error {
				...
				if b, ok := data["isCreatedWithAutoGeneratePassword"]; ok {
					isCreated = b.(bool)
				}
				if !isCreated || user.GetStatus().GetObservedGeneration() == 0 {
					return errors.New("Not yet created")
