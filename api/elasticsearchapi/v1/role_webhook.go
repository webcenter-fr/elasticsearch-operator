...
// +kubebuilder:webhook:path=/validate-elasticsearchapi-k8s-webcenter-fr-v1-role,mutating=false,failurePolicy=fail,sideEffects=None,groups=elasticsearchapi.k8s.webcenter.fr,resources=roles,verbs=create;update,versions=v1,name=role.elasticsearchapi.k8s.webcenter.fr,admissionReviewVersions=v1
...
func (r *roleValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	roletObj, ok := obj.(*Role)
	if !ok {
		return nil, fmt.Errorf("expected a Role object but got %T", obj)
	}
	r.logger.Debugf("validate create %s/%s", roletObj.GetNamespace(), roletObj.GetName())

	if err := roletObj.Spec.ElasticsearchRef.ValidateField(); err != nil {
		allErrs = append(allErrs, err)
	}

	if err := r.validateResourceUnicity(roletObj); err != nil {
		allErrs = append(allErrs, err)
	}

	if len(allErrs) > 0 {
		return nil, apierrors.NewInvalid(
			roletObj.GroupVersionKind().GroupKind(),
			roletObj.Name, allErrs)
	}
...
func (r *roleValidator) ValidateUpdate(ctx context.Context, oldObj runtime.Object, newObj runtime.Object) (admission.Warnings, error) {
	...
	roletObj, ok := newObj.(*Role)
	if !ok {
		return nil, fmt.Errorf("expected a Role object but got %T", newObj)
	}
	r.logger.Debugf("validate update %s/%s", roletObj.Namespace, roletObj.Name)
	...
	if err := validateImmutableName(roletObj, oldO); err != nil {
		allErrs = append(allErrs, err)
	}
